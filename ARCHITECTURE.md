...
| Someone seeing the app itself on your device | **Partly** | The encryption protects what you record, not the fact that MyHRT is installed. Since 1.2.0, discreet mode (Settings > Privacy > App icon and name, Android only) swaps the launcher name and icon for a neutral preset, which also carries into the recent-apps list and the lock-screen title. That hides the app from a glance, not from a search: the real label and the package name (`health.myhrt`) remain visible under Settings > Apps, in the app-info page, on Google Play, in the widget picker and in the header Android stamps on every notification, and any app allowed to list installed packages can see it. The other privacy settings (generic notification text and a plain bell icon, hidden widget names, screenshot blocking) reduce what appears on a locked or shared screen. |
